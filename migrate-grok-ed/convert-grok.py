...
KEYS_WITH_DUMPS_VALUES = ('workspace', 'tests', \
'solutions', 'options', 'choices', 'blockly_blocks')
...
FILETYPE_NAMES = {0: "stdio", 1: "stdout", 2: "stderr", 3: "stdin", 4: "Normal In", 5: "Normal Out", 22: "multichoice"}
...
class Problem():
    ...
    def __init__(self, ex):
        self.ex = ex
        self.wd = Path(ex) #Path(ex.replace('.', '-'))
        self.load_json()
    ...
    def test(self):
        ...
        for solution in glob.glob(f"{self.wd}/solutions/*"):
            ...
            out = subprocess.run(['make', 'build'], cwd=wd,
            ...
            for test_no, test in enumerate(self.obj['tests']['tests']):
                ...
                print(
                    f"Testing #{test_no}: {test['onpass']} ({test['label']}) ... ", end="")
                ...
                out = subprocess.run(['make', '-s', 'run'], input=stdin, cwd=wd,
        ...
        print("--", ok("✓ all tests passed")
              if passed else fail("✗ some tests failed"))
        return passed
    ...
    def load_solutions(self):
        ...
        for i, path in enumerate(glob.glob(f"{self.wd}/solutions/*")):
            old_solution = None if i >= len(
                old_solutions) else old_solutions[i]
            self.obj['solutions'].append(
                load_one_workspace(path, old_solution))
    ...
    def diff_json(self):
        ...
        print("diff:")
        print(json.dumps(diff(old_json, self.obj, syntax='symmetric',
                              marshal=True), indent=4, sort_keys=True))
    ...
    def diff_yaml(self):
        ...
        print(dbg("\n".join(difflib.unified_diff(
            old_yaml_txt, yaml_txt, n=0, lineterm=""))))
...
def parse_args():
    ...
    parser.add_argument('-r', action='store_true',
                        help="read json and overwrite local files")
    ...
    parser.add_argument('-d', action='store_true',
                        help="diff only (json vs current work directory)")
